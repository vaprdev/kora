defmodule Kora.InterceptorTest do
	...
	test "validate mutation" do
		trigger(:validate_mutation, :ok)
	end
...
defmodule Kora.InterceptorTest.Interceptor do
	...
	def validate_mutation(["a", "c"], _user, _layer, _mutation) do
		{:error, :failed}
	end 

	def validate_mutation(["a", "b"], _user, _layer, mutation) do
		IO.inspect("wtf")
		IO.inspect(mutation)
		:ok
	end
